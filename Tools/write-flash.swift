// write-flash.swift — Flash writer for ESP32-C3 via serial
...
class ESPLoader {
    ...
    func flashId() throws -> UInt32 {
        ...
        // For ESP32-C3: SPI_MEM_C base = 0x60002000
    ...
    // ESP32-C3 ROM requires 5th param (encrypted flag) since SUPPORTS_ENCRYPTED_FLASH=True
...
func md5Hex(_ data: Data) -> String {
    md5Digest(data).map { String(format: "%02x", $0) }.joined()
}
...
do {
    ...
    // Enable SPI flash (required for ROM loader on ESP32-C3)
    ...
    // Hard reset via USB-JTAG-Serial glue logic (chip_reset = RTS & ~DTR)
