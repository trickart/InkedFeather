// image-info.swift — ESP32-C3 image header info display
// Usage: swift image-info.swift <image.bin>

import Foundation
import CommonCrypto

// MARK: - Constants (ESP32-C3 memory map)

let IROM_MAP_START: UInt32 = 0x42000000
let IROM_MAP_END:   UInt32 = 0x42800000
let DROM_MAP_START: UInt32 = 0x3C000000
let DROM_MAP_END:   UInt32 = 0x3C800000
let IRAM_START:     UInt32 = 0x4037C000
let IRAM_END:       UInt32 = 0x403E0000
let DRAM_START:     UInt32 = 0x3FC80000
let DRAM_END:       UInt32 = 0x3FCE0000
...
let chipIdNames: [UInt16: String] = [0: "ESP32", 2: "ESP32-S2", 5: "ESP32-C3", 9: "ESP32-S3", 12: "ESP32-C2", 13: "ESP32-C6"]
...
func memoryType(_ addr: UInt32) -> String {
    ...
    if IROM_MAP_START <= addr && addr < IROM_MAP_END { types.append("IROM") }
    if DRAM_START <= addr && addr < DRAM_END { types.append("DRAM") }
...
let chipName = chipIdNames[chipId] ?? "Unknown(\(chipId))"

print("File size: \(data.count) (bytes)")
print("Chip: \(chipName) (id=\(chipId))")
...
for i in 0..<segmentCount {
    ...
    print(String(format: "Segment %d: len 0x%05x load 0x%08x file_offs 0x%08x [%@]",
                 i + 1, size, addr, offset, memoryType(addr)))
