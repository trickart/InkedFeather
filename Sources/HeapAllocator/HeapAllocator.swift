...
/// Return total free bytes and largest free block size.
@c(heap_stats)
func heapStats(
    _ totalFree: UnsafeMutablePointer<UInt>,
    _ largestBlock: UnsafeMutablePointer<UInt>
) {
    ...
    while curr != 0 {
        let sz = load(curr)
        let usable = sz &- headerSize &- footerSize
        total &+= usable
        if usable > largest { largest = usable }
        curr = load(curr &+ headerSize)
    }
    totalFree.pointee = total
    largestBlock.pointee = largest
}
